...
#%% Ablation study and comparison with Random Jitter


# Load Cross-validation results

basepath = Path('./results/additional_experiments/ablation')
...
#Geometric mean rank of augmentation methods
...
plt.xticks(rotation = 45, ha = 'right')
plt.title(title)
#savefig("Table_ablation" + metric )
plt.show()

# Number of datasets for which each augmentation tool has best metric value
...
fig, axes = plt.subplots(nrows = 3, ncols = len(metric), sharex = False,
                         figsize = (4 * len(metric), 6), squeeze = False,
                         gridspec_kw = {"height_ratios": [18, 1, 1],
                                        "wspace": 0.05,
                                        "hspace": 0.1})
for i, m in enumerate(metric):
    classifiers = mean_metrics[m].index.levels[0]
    ...
    count = (rank == 1).groupby(level = 0, sort = False).sum().T
    mean_rank = rank.groupby(level = 0, sort = False).mean().T
    #count = count.rename({'Organic': 'Baseline'})
